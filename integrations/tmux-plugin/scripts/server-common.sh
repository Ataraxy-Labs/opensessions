...
TMUX_OPENSESSIONS_NODE_ID="$(tmux show-environment -g OPENSESSIONS_NODE_ID 2>/dev/null | cut -d= -f2-)"
TMUX_OPENSESSIONS_TMUX_SOCKETS="$(tmux show-environment -g OPENSESSIONS_TMUX_SOCKETS 2>/dev/null | cut -d= -f2-)"
TMUX_OPENSESSIONS_CLOUD_URL="$(tmux show-environment -g OPENSESSIONS_CLOUD_URL 2>/dev/null | cut -d= -f2-)"
TMUX_OPENSESSIONS_REMOTE_ATTACH_COMMANDS="$(tmux show-environment -g OPENSESSIONS_REMOTE_ATTACH_COMMANDS 2>/dev/null | cut -d= -f2-)"
TMUX_OPENSESSIONS_SSH_ATTACH_WRAPPER="$(tmux show-environment -g OPENSESSIONS_SSH_ATTACH_WRAPPER 2>/dev/null | cut -d= -f2-)"
TMUX_OPENSESSIONS_CLOUD_API_KEY="$(tmux show-environment -g OPENSESSIONS_CLOUD_API_KEY 2>/dev/null | cut -d= -f2-)"
...
ensure_server() {
  ...
  OPENSESSIONS_NODE_ID="$TMUX_OPENSESSIONS_NODE_ID" \
  OPENSESSIONS_TMUX_SOCKETS="$TMUX_OPENSESSIONS_TMUX_SOCKETS" \
  OPENSESSIONS_CLOUD_URL="$TMUX_OPENSESSIONS_CLOUD_URL" \
  OPENSESSIONS_REMOTE_ATTACH_COMMANDS="$TMUX_OPENSESSIONS_REMOTE_ATTACH_COMMANDS" \
  OPENSESSIONS_SSH_ATTACH_WRAPPER="${TMUX_OPENSESSIONS_SSH_ATTACH_WRAPPER:-$PLUGIN_DIR/integrations/tmux-plugin/scripts/ssh-attach-wrapper.sh}" \
  OPENSESSIONS_CLOUD_API_KEY="$TMUX_OPENSESSIONS_CLOUD_API_KEY" \
